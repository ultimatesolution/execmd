...
func (c *Cmd) RunWithInput(input, command string) (res CmdRes, err error) {
	if res, err = c.StartWithInput(input, command); err != nil {
		return
	}
...
func (c *Cmd) StartWithInput(input, command string) (res CmdRes, err error) {
	args := []string{}
	if c.Interactive {
		args = append(args, "-i")
	}
